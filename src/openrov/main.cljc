(ns openrov.main
  "Kotodama WASM entrypoint for the OpenROV clean-room actor (L4) — Clojure port.
  ...
  facts (`openrov.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "openrov")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"openrov.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openrov-compat" :tier tier :entities entities} 200])
...
(defn start! [] :openrov-compat/ready)
